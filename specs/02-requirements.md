...
| FR-003 | The GeoComp menu MUST present eight entries in this order: Total Station, Level, GNSS, Gravimetry, Integration, Analysis, Project, and Global Settings, the last separated from the rest. | tex §Painel de Configuração Global; `fig/menu_estrutura.png` (five technique submenus and Global Settings); Analysis added in phase P2 for the operations belonging to no single technique, settling what [`15-ui-menu-and-settings.md`](./15-ui-menu-and-settings.md) §1.1 left open; Project added in phase P5 for operations on a project's results rather than on one technique's observations |
| FR-004 | Menu groups Total Station, Level, GNSS, Gravimetry, Integration, Analysis and Project MUST each open a submenu listing the operations specified in [`15-ui-menu-and-settings.md`](./15-ui-menu-and-settings.md). | tex §Painel de Configuração Global |
...
| NFR-008 | The adjustment MUST be correct at every network size it accepts, using only NumPy. Beyond roughly 2,000–3,000 stations it MUST use a sparse factorisation, which requires SciPy; where SciPy is absent and the network exceeds what the dense path can hold, the adjustment MUST refuse with a message naming SciPy rather than exhausting memory. Networks of at least 10,000 stations MUST be supported in file mode with SciPy present; beyond that, DynAdjust segmentation is the supported path. | derived from FR-220 (bounds the in-house core against the engine's scale, per `tex §O Software DynAdjust`); reworded in phase P2, see [`adr/0008-scipy-and-network-scale.md`](./adr/0008-scipy-and-network-scale.md) |
