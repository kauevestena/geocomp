...
class NetworkPreAnalysisAlgorithm(GeoCompAlgorithm):
        ...
        with open(path, "w", encoding="utf-8", newline="") as handle:
            ...
            for station in design.stations:
                writer.writerow(
                    [
                        station.station_id,
                        exact(station.positional_uncertainty),
                        exact(station.ellipse.semi_major),
                        exact(station.ellipse.semi_minor),
                        exact(station.ellipse.orientation),
                    ]
                )
    ...
    def _render(self, network, frame, datum, design, confidence, tolerance, meets) -> str:
        ...
        return render_document(
            ...
            body,
            footer=escape(
                self.tr("Generated by GeoComp — geocomp:analysis_network_preanalysis")
            ),
        )
