...
from geocomp.algorithms.totalstation.common import findings_table, load_json
...
class ClassicalNetworkAlgorithm(GeoCompAlgorithm):
    ...
    def processAlgorithm(
        ...
        reductions = load_json(
            self.parameterAsFile(parameters, REDUCTIONS, context), parameter=REDUCTIONS
        )
        if reductions.get("kind") != "geocomp.reductions":
            raise QgsProcessingException(
                self.tr(
                    "That file is not a GeoComp reductions document. Run Generalised "
                    "pre-processing first, or choose the file it produced."
                )
            )
...
def _setup_results(document: dict[str, Any]):
    """Rebuild the minimal setup results ``build_network`` needs.

    Only the fields the network builder reads are reconstructed. A full
